...
func reverseSearchHandler(c *gin.Context) {
	...
	systemPrompt := `You are an expert on Kolkata's heritage and streets.
Identify the location in the provided image.
Choose the closest match from this exact list: College Street, Howrah Bridge, Kumartuli, Maidan Tram Depot, North Kolkata, Park Street, Rabindra Sarani, Jorasanko, Shyambazar, Esplanade.
Reply with ONLY the location name and nothing else.`

	req := ChatRequest{
		Model: "gpt-4o", MaxTokens: 50, Temperature: 0.1,
		Messages: []ChatMessage{
			{Role: "system", Content: systemPrompt},
			{Role: "user", Content: []ContentPart{
	...
	identifiedLocation := strings.TrimSpace(chatResp.Choices[0].Message.Content)
	locName, lat, lng := nearestLocation(identifiedLocation)
	c.JSON(http.StatusOK, gin.H{"location": locName, "lat": lat, "lng": lng})
}
...
func miroHandler(c *gin.Context) {
	...
	req, _ := http.NewRequest("POST", "https://api.miro.com/v2/boards", bytes.NewReader(boardReqJSON))
	...
	// 2 — Title card (text widget, top-centre)
	...
	req2, _ := http.NewRequest("POST",
		fmt.Sprintf("https://api.miro.com/v2/boards/%s/text", boardResp.ID),
		bytes.NewReader(titleCard))
	req2.Header.Set("Authorization", "Bearer "+apiKey)
	req2.Header.Set("Content-Type", "application/json")
	client.Do(req2)

	// 3 — Main content as sticky note(s), split if too long
	content := result.Content
	...
	for i, chunk := range chunks {
		...
		req3, _ := http.NewRequest("POST",
			fmt.Sprintf("https://api.miro.com/v2/boards/%s/sticky_notes", boardResp.ID),
